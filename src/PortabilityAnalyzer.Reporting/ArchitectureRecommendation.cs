    ...
    public static ArchitecturePlan Build(AnalysisReport report, Lang lang = Lang.Es)
    {
        ...
        var projects = new List<RecommendedProject>
        {
            ...
            new($"{baseName}.Abstractions", "net8.0", Loc.T(lang,
                "Capa de abstracción GENERADA por la reescritura: interfaces de las capacidades que dependen del SO + una implementación MULTIPLATAFORMA por defecto de cada una, registradas por DI (AddPortableAbstractions). El núcleo depende solo de las interfaces.",
                "Abstraction layer GENERATED by the rewrite: interfaces for OS-dependent capabilities + a CROSS-PLATFORM default implementation of each one, registered via DI (AddPortableAbstractions). The core depends only on the interfaces."))
        };
        if (hasUi)
