...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            // anade al catalogo las reglas nuevas (marcadas para revisar) y NO ejecuta el analisis.
            if (options.DiscoverRules)
            {
                ...
                if (added < 0) return 2;
                Log.Information("Descubrimiento COMPLETADO: {Added} regla(s) nueva(s) anadida(s) a {Rules}. Revisa las marcadas 'REVISAR' y vuelve a ejecutar SIN --discover-rules.",
            ...
            var report = new AnalysisReport
            {
                ...
                ReferencedLibraries = ProjectDiscovery.Handles(options.InputPath) && File.Exists(options.InputPath)
                    ? LibraryUsageAnalyzer.Build(new ProjectDiscovery().GetProjects(options.InputPath))
