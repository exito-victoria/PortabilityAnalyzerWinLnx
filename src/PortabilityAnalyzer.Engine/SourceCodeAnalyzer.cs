...
public sealed class SourceCodeAnalyzer
{
    ...
    private static string Fix(string categoria) => categoria switch
    {
        ...
        "Threading" => "El modelo de hilos del BCL (Thread, Task, Parallel, async/await, ThreadPool, SemaphoreSlim, System.Threading.Timer) ya es multiplataforma. Solo cambia lo de WPF: DispatcherTimer -> shim portable PortableTimer (System.Timers.Timer); Dispatcher.Invoke/BeginInvoke -> async/await + IProgress<T>. STAThread es no-op fuera de Windows.",
